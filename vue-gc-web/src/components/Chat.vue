...
export default {
  data () {
    return {
      ws: null,
      count: 0,
      userId: JSON.parse(window.localStorage.getItem('adminAndUser')).data.id, // 当前用户ID
      username: JSON.parse(window.localStorage.getItem('adminAndUser')).data.uname, // 当前用户昵称
  ...
  methods: {
    ...
    initWebSocket () {
      ...
      if (window.WebSocket) {
        ...
        const sip = '房间号'
        // 填写本地IP地址 此处的端口号 要与后端配置的一致！
        var url = 'ws://' + serverHot + ':8081' + '/groupChat/' + sip + '/' + this.userId // `ws://127.0.0.1/9101/groupChat/10086/聊天室`
        console.log('usserid', _this.userId)
        console.log(url)
